...
app.get('/books/que1', function(req, res) {
  Book.find({
      pages: {$gt: 200 , $lt: 500}, 
...
app.get('/books/que2', function(req, res) {
  Book.find({
    rating: {$lt : 5}
  })
  .sort({author: -1})
...
app.get('/books/que3', function(req, res) {
  Book.find({
    genres: "Fiction"
  })
  .skip(2)
...
app.get('/person/que1', function(req, res) {
  Person.find({
      height: {$gt: 180},
      salary: {$gt: 30000}
    }, 
...
app.get('/person/que2', function(req, res) {
  Person.find({
    $or: [
      {height: {$gt: 180}},
      {salary: {$gt: 30000}}
...
app.get('/person/que3', function(req, res) {
  Person.find().and([
    { $or: [
      {hair: "grey"},
      {eyes: "grey"}
...
app.get('/person/que4', function(req, res) {
  Person.find({
    "kids.hair": "grey"
  },
    function(err, results){
...
app.get('/person/que5', function(req, res) {
  Person.find({
    $and: [
      {"kids.weight": {$gt: 100}},
      {weight: {$gt: 100}}
